"""
数据工程：简言之，对原始数据进行精细化加工，使其更便于或更有利于解决实际问题。

在机器学习领域，一个好的模型往往基于三点：【1】足量的优质数据。【2】准备好的数据工程。【3】模型的选择。 其中准备好的数据工程可以具体概括为：基于样本（观测/个案）和属性（变量）所构成的数据集
...
#（1.5）Bootstrap重采样
"""
Bootstrap重采样理解起来很简单：即在现有样本的基础上，重新多次（1000次以上）采样（当然要求重采样样本量小于原样本量）。
但就是这么一个简单的技术却能有效帮助得到估计量的经验分布! 从而经验地得到关于待估参数的更稳定的估计！
这比统计推断中计算估计量分布函数的方法更简单更实际。

Bootstrap重采样主要用于小样本情形，用于解决样本不足的问题。而较大样本情形下就不需要了，更常见的是交叉验证的技术。两者
本质上同属一脉，均为重采样技术，在实际机器学习中，后者使用更为常见。
"""
import numpy as np

X=np.random.normal(0,1,100) # 获取均值0，方差1的100个正态“小样本”。
print(np.mean(X)) # 得到均值的估计一个

A=np.array([])
for i in range(1000):
    XX=np.random.choice(X, size=80, replace=True) # 随机抽取80个样本，重复1000次并计算均值
    A=np.append(A, np.mean(XX))
