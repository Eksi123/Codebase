...
A6=np.append(A6_0,A6_1,axis=0) # 两个数组按列拼接，axis=1则为按行拼接，也可用于添加新元素
...
"""
A10=np.random.choice(range(100), size=60, replace=True) # 对某数组随机抽取60个样本，replace=True表示不放回
...
pyplot.hist(X) # 查看直方图，观察可知所取样本来自于正态分布N(mean=0, variance=1)
pyplot.show() 
...
data1=pd.read_csv("1.csv",header=0,encoding="utf8")# 导入数据表，以csv文件为例
...
data1[data1["age"]>=13]="old"; data1[data1["age"]<13]="young" # 将age列重编码
