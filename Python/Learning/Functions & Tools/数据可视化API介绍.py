...
# 4 Pandas绘图补充
"""
考虑到在matplotlib绘图工作较底层，需花费较大精力构图，因而在调用DateFrame, Series两种类型数据集时，我们主要使用
Pandas模块自带的绘图函数，其突出优点在于绘图指令较少，且多幅子图的绘制十分方便。

在实际应用中，常常通过绘图来观察数据的分布情况和相关情况，因此一般只需了解箱线图，直方图，核密度图，散点图几类即可。
"""
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
...
df=pd.DataFrame(  # 创建数据框
     {"X1":np.random.binomial(10,0.6,size=1000),
     "X2":np.random.normal(0,1,size=1000),
     "X3":np.random.uniform(size=1000),
     "X4":np.random.exponential(0.2,size=1000)},
     columns=["X1","X2","X3","X4"]
)

# 我们以直方图为例作展示
df.plot.hist(sharex=True, sharey=True) # sharex和sharey指是否共享坐标X和Y
df.plot.hist(subplots=True, layout=(2,2), sharex=False, sharey=False) # subplots指是否根据变量绘制子图，layout指子图排布方式
plt.show()
"""
其余绘图函数为：
箱线图：df.plot.box()
核密度图：df.plot.density()
散点图：df.plot.scatter(x="", y="")
散点密度图：df.plot.hexbin(x="", y="")
"""

# 另一个常用的函数为pd.plotting.scatter_matrix()，用于绘制散点矩阵图：
pd.plotting.scatter_matrix(df, diagonal="density") # diagonal="density"表示主对角线上为密度图，默认直方图
plt.show()