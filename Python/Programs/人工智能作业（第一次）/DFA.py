...
def Str_Find(file_name): # 用于主子字符串匹配并输出对应行数
    document = open(file_name, "r", encoding="utf-8")  # 读取外部文件（建议txt文件与源程序放同一目录下，否则file_name还得包含路径名
    sub_string = document.readline().rstrip().split(sep=":")[1]
    rows = 0
    while True:
        main_string = document.readline().rstrip()
        if main_string == "": # 当读取至最后一行时，退出（注意文档中别出现空行）
            break
        main_string = main_string.split(sep=":")[1]
        rows = rows+1
